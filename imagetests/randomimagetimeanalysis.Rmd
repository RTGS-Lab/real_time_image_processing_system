...
scripts <- c(mean(time1$script), mean(time2$script), mean(time3$script), mean(time4$script), mean(time5$script))
flasks <- c(mean(time1$flask), mean(time2$flask), mean(time3$flask), mean(time4$flask), mean(time5$flask))
difs <- c(dif1, dif2, dif3, dif4, dif5)
plot(flasks ~ dimensions, ylim=c(0, 1.3))
points(scripts ~ dimensions)
